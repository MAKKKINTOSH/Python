...
DB=DeadDb('E:\Рабочий стол\github\Python\Telegram_bots\deadlines bot\deadline_data_base.db')
...
def main():
    ...
    def Choise(call):
        #print("***", call.data, "***")             DELETE ON RELEASE

        CallData = ['InProf', 'Nosyreva', 'English', 'Informatics', 'Math', 'BBC', 'Prog', 'Physics', 'PE']
        global edit_access, ObjectType, DeadlineDate, DeadlineType
        ...
        if call.data == 'ToMenu':
            ...
            except: ''                                                                          #РЕШИТЬ ПРОБЛЕМУ ОБЯЗАТЕЛЬНО Bad Request: message can't be deleted for everyone
            finally:
                Menu()

        if call.data == '1' or call.data == '0':
            DeadlineType = call.data
    ...
    def delete_or_make(message):
        #print(edit_access)         DELETE ON RELEASE
        if edit_access == 2:
            n = message.text
            try:
                ...
                #print(edit_access, ObjectType, message.text)       DELETE ON RELEASE
                Menu(message)
        if edit_access == 1:
            a = message.text
            try:
                DeadlineDate = a[6] + a[7] + a[8] + a[9] + '-' + a[3] + a[4] + '-' + a[0] + a[1]
                DB.make_deadline(ObjectType, int(DeadlineType), DeadlineDate)
